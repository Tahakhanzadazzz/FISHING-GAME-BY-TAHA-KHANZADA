...
.data
	...
	prompt BYTE "Press Space when the arrow aligns with the [X] to successfully hook up!", 13, 10, 13, 10, 0
	blocks BYTE "[ ][ ][ ][ ][ ][ ][ ][ ]", 0
...
; Returns:  EAX - The level of fish caught, or 0 if failed
	...
	INVOKE getMin, [Level], 15			;If fish level exceeds lvl15, set it to lvl15
	MOV Level, EAX

	MOV EAX, 40
	MUL Level
	MOV EBX, 640          ;Base (Slowest) time is 600ms
	SUB EBX, EAX
	MOV DelayTime, EBX
...
spacebar_pressed:
	...
	; Revert changes to the block string
	MOV EAX, RandomPosition
	MOV [blocks+EAX], ' '
